...
package com.hit.wangoceantao.amino.alg.parallelprefix;
...
public class ThreadedParallelPrefix<T> extends AbstractParallelPrefix<T> {
	...
	// FIXME: find a better number than this guess.
	public static final int DEFAULT_MINIMUM_ARRAY_SIZE = 10000; // minimum size
	...
	public ThreadedParallelPrefix(int nthreads) {
		...
		_minArraySize = DEFAULT_MINIMUM_ARRAY_SIZE;
	}
	...
	public ThreadedParallelPrefix() {
		_numberThreads = Runtime.getRuntime().availableProcessors();
	}
	...
	public class PartialReduction {
		...
		public int[] reduce(final int[] inputArray, final int[] reducedArray,
				final BinaryOp<T> op) {
			int numThreads = com.hit.wangoceantao.amino.Runtime.reserveThreads(_numThreads);
	...
	public void scan(int[] inputArray, int[] outputArray, BinaryOp<T> op) {
		...
		int numThreads = _numberThreads;
		if (inputArray.length > _numberThreads)
